...
| Mode | Advisors | Peer Review | Word Target | Spawns | Token Est. |
|------|----------|-------------|-------------|--------|------------|
| **Lite** | 3 (Contrarian, First Principles, Executor) | No | ~100 each | 4 | ~2,000 out |
| **Full** | 5 | Yes | ~150 each | 11 | ~6,000 out |
| **Deep** | 5 | Yes | ~200 each + full review | 11 + HTML gen | ~9,000 out |
...
**Lite mode uses only:** Contrarian, First Principles, Executor. The two advisors dropped (Expansionist, Outsider) are the ones most likely to overlap with normal brainstorming energy and casual analysis patterns.
...
```
Check: /mnt/c/Users/pierc/ObsidianVault/Memo/council-index.md
Check recent: /mnt/c/Users/pierc/ObsidianVault/Memo/Agent-OpenClaw/daily/  (last 7 days)
```

If the same or near-identical question was counciled recently, surface the previous transcript and ask if dad wants to rerun or just revisit the existing verdict.
...
Save to: `/mnt/c/Users/pierc/ObsidianVault/Memo/council-[timestamp].md`
...
- All advisor responses (identified by role, not anonymized for the vault copy)
- Peer reviews (Full/Deep only)
- Chairman synthesis + verdict

### Step 9: Update Council Index (Deep only)

After a Deep council, append to `/mnt/c/Users/pierc/ObsidianVault/Memo/council-index.md`:
```
## YYYY-MM-DD: [one-line question summary]
Mode: deep | Advisors: C, FP, Ex, En, Out | Verdict: [one line]
File: council-YYYY-MM-DD-[slug].md
```

Lite and Full sessions don't need index entries -- they're too frequent. Deep sessions are the ones worth indexing for cross-reference.
...
**Chairman verdict:** The consulting meeting is likely a rabbit hole. The university IT decision cycle is 6-12 months. Unless there's a specific deliverable that maps to current needs, it's a time trap. Flag it to the Executor for a "should I go at all" reframe.
...
*Full 5+5+1 run. Vault transcript saved with full history. HTML report generated. Council index updated.*
...
- **Model selection is per-advisor.** Different models = genuinely different perspectives. Using the same model for all 5 advisors defeats the purpose of having distinct roles.
- **Lite mode is the default for ambiguous triggers.** If it feels like a council but no tier specified, default to lite unless the stakes are clearly high.
- **Duplicate detection saves time and token waste.** A council re-run should confirm there's new information or changed circumstances before spinning up the full machine.
- **The vault is the source of truth for transcripts.** HTML reports are nice-to-have for Deep mode only.
